...
export function parseBankAccount(bankAccount) {
  let result = 0,
    account = bankAccount.split('\n');
  for (let i = 0; i < 9 * 3; i += 3) {
    let num;
    if (account[0][i + 1] === ' ') {
      if (account[1][i + 1] === '_') num = 4;
      else num = 1;
    } else if (account[2][i + 2] === ' ') num = 2;
    else if (account[1][i] === ' ') {
      if (account[1][i + 1] === '_') num = 3;
      else num = 7;
    } else if (account[1][i + 2] === ' ') {
      if (account[2][i] === ' ') num = 5;
      else num = 6;
    } else if (account[2][i] === ' ') num = 9;
    else if (account[1][i + 1] === ' ') num = 0;
    else num = 8;
...
export function* wrapText(text, columns) {
  ...
  while (result.length) {
    if (
      result.length > 1 &&
      result[0].length + result[1].length + 1 <= columns
    ) {
      result[0] += ' ' + result[1];
      result.splice(1, 1);
    } else {
